...
public class UserService {
    ...
    public UserInfoDTO userCreate(UserPrivacyDTO userDTO) {
        ...
        if(userDTO.getRoles().contains(SERVICE)){
            String jwtToken = JWT.create()
                    ...
                    .withExpiresAt(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * 24 * 30 * 12 * 10)) // 만료시간 10년
            ...
            userDTO.setServiceToken(Collections.singletonList(jwtToken));
        }
    ...
    public UserInfoDTO userUpdate(UserInfoDTO dto){
        UserEntity user = userRepository.findByUsername(dto.getOldName());
        if (!dto.getUsername().equals(dto.getOldName())) userRepository.deleteById(dto.getOldName());
        user.change(dto);
